...
ip_address_ender = 41 # ip address ender for front limelight
totalTime = 5.0  # how long you want the video to be (in seconds)
firstTime = True

cap = cv2.VideoCapture('http://10.44.99.41:5800', cv2.CAP_FFMPEG) # starts the video capture of stream using ffmpeg as backend
...
fourcc = cv2.VideoWriter_fourcc(*'mp4v') # sets the fourcc codec for video stream (XVID for .avi, mp4v for .mp4)
...
second = splitList[5]
decimal = splitList[6]
videoName = 'LL' + str(ip_address_ender) + 'Video-' + year + 'Y-' + month + 'M-' + day + 'D-' + hour + 'h-' + minute + 'm-' + second + 's.mp4' # final name for video file, currentlt set as .mp4 type

out = cv2.VideoWriter(videoName, fourcc, 15.0, size, True) #creates writer object
